...
const businessID=location.pathname.split("/")[2]
  ...
  const handleSubmit = (event) => {
    ...
    if (
      ...
      axios.put('http://localhost:8085/business/'+businessID, values)
        .then(res => {
          console.log('Business updated successfully:', res.data);
        ...
        <div className='p-5'>
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-3">
              ...
              <input type="text" id="service" name="service" placeholder='service' value={values.service} onChange={handleInput} className='form-control' />
              {errors.service && <span className='text-danger'>{errors.service}</span>}
            </div>
            <div className="mb-3">
              <label htmlFor="description"><strong>Description:</strong></label>
              <input type="text" id="description" name="description" placeholder='desc' value={values.description} onChange={handleInput} className='form-control' />
              {errors.description && <span className='text-danger'>{errors.description}</span>}
            </div>
            <button type='submit' className='btn btn-success w-100' style = {{backgroundColor:'#1D2634'}} ><strong>Update Business</strong></button>
          </form>
        </div>
      </div>
    </div>
